...
export default function KnowledgePanel({ threadId }: Props) {
  const id = threadId;
  ...
  function navItem(label: string, item: SelectedItem, badge?: string, indent = 0) {
    const isActive =
      selected &&
    ...
    return (
      ...
      >
        {badge && <span className="text-zinc-600 w-5 shrink-0">{badge}</span>}
          ...
          <div className="w-48 shrink-0 border-r border-zinc-800 overflow-y-auto py-2">
            ...
            {archives.l1.length > 0 && (
              <>
                ...
                {archives.l1.map((e, i) => navItem(e.name, { kind: 'l1', index: i }, 'L1'))}
              </>
            )}
            ...
            {archives.l0.length > 0 && (
              <>
                ...
                {archives.l0.map((e, i) => navItem(`Op ${e.id}`, { kind: 'l0', index: i }, 'L0'))}
              </>
            )}
